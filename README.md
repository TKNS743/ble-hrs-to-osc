...
### トークンの設定方法
- 1.2.3.のいずれかで設定します。1.の`pulsoid_token.txt` を使う方法がおすすめです。

#### 1. `pulsoid_token.txt` を使う（おすすめ）

1. `pulsoid_token.txt.example` をコピー
2. `pulsoid_token.txt` にリネーム
...
#### 3. 環境変数で設定（Windows）(非推奨)

```cmd
set PULSOID_ACCESS_TOKEN=ここにトークン  REM 一時的にこのウィンドウで有効
```

参考:`pulsoid_to_osc.py` では、次の順番でトークンを探します。
No.  |取得方法
-----|---------------------------
...
- 出力形式は`hrs_to_osc.py`に準じます。

### Getting Started(for Pulsoid)

- 基本的には`hrs_to_osc`本体と同じです。
- Install Dependenciesについては`for_pulsoid`ディレクトリ内の`requirements.txt`を指定してください。
- RunするPythonファイルは`for_pulsoid`ディレクトリ内の`pulsoid_to_osc.py`となります。
